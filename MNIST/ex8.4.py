...
class MNIST_Test:
    def __init__(self, input_nodes, hidden_nodes, output_nodes, learning_rate):
        ...
        # 출력층 가중치 W2 Xavier/He 방법으로 self.W2 가중치 초기화
        self.W3 = np.random.randn(self.hidden_nodes, self.output_nodes) / np.sqrt(self.hidden_nodes/2)
        self.b3 = np.random.rand(self.output_nodes)

        print("MNIST_Test objet is created !!!")
    ...
    # 수치 미분을 이용하여 손실 함수가 최소가 될 떄까지 학습하는 함수
    ...
    def predict(self, input_data):
        ...
        # 0 또는 1이 아닌 argmax()를 통해 최대 인덱스를 ㅋ넘겨 주어야 함
...
for step in range(epochs):
    for index in range(len(training_data)):
        ...
        # input_data, target_data normaliaze
        input_data = ((training_data[index, 1:] / 255.0) * 0.99 + 0.01)
        target_data = np.zeros(o_nodes) + 0.01
        target_data[int(training_data[index, 0])] = 0.99 # 1이 아닌 이유? 0.01 ~ 1까지 input_data 인데 target_data는 0.01 ~ 0.99까지인 이유누누?
